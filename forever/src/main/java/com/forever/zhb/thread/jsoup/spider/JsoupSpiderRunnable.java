...
public class JsoupSpiderRunnable implements Runnable {
    
    private Logger logger = LoggerFactory.getLogger(JsoupSpiderRunnable.class);
    ...
    public JsoupSpiderRunnable(String url,String targetSavePath,int nameIndex,int begin,int end) {
        this.url = url;
            ...
            if (null != imagePaths) {
                for (String urlPath : imagePaths) {
                    try {
                        ...
                        DownloadURlUtil.downLoadFromUrl(urlPath, fileName, targetSavePath);
