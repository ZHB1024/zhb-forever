package com.forever.zhb.utils.attachment.pdf;

import java.util.Calendar;

public class ApplicationData {

    private String id;
    private String userId;
    private Calendar createTime;
    private Calendar updateTime;
    private String bz;
    private String status;
    private String orderId;
    private String lqfs;
    private String lxrxm;
    private String lxdh;
    private String province;
    private String city;
    private String district;
    private String lxdz;
    private String yzbm;
    private Calendar applyTime;
    private String phone;
    private String xm;
    private String xb;
    private String cc;
    private String sfzh;
    private String txzh;
    private String txzfzjg;
    private String lqnf;
    private String rxrq;
    private String yxmc;
    private String zymc;
    private String byrq;
    private String bjyjl;
    private String xxzjmc;
    private String xxzjhm;
    private String byzsmc;
    private String zsbh;
    private String zmnf;
    private String zmwm;
    private String zmwh;
    private String shjl;
    private String shyj;
    private boolean change;
    private Calendar dysj;
    private Calendar bjsj;
    private String receivePaper;
    private String shyy;
    private String yjzqsj;
    private String exCompany;
    private String exNumber;
    private String czrbz;
    private Calendar shsj;
    private Calendar bzsj;

    public String getUserId() {
        return userId;
    }

    public void setUserId(String userId) {
        this.userId = userId;
    }

    public Calendar getCreateTime() {
        return createTime;
    }

    public void setCreateTime(Calendar createTime) {
        this.createTime = createTime;
    }

    public Calendar getUpdateTime() {
        return updateTime;
    }

    public void setUpdateTime(Calendar updateTime) {
        this.updateTime = updateTime;
    }

    public String getBz() {
        return bz;
    }

    public void setBz(String bz) {
        this.bz = bz;
    }

    public String getStatus() {
        return status;
    }

    public void setStatus(String status) {
        this.status = status;
    }

    public String getOrderId() {
        return orderId;
    }

    public void setOrderId(String orderId) {
        this.orderId = orderId;
    }

    public String getLqfs() {
        return lqfs;
    }

    public void setLqfs(String lqfs) {
        this.lqfs = lqfs;
    }

    public String getLxrxm() {
        return lxrxm;
    }

    public void setLxrxm(String lxrxm) {
        this.lxrxm = lxrxm;
    }

    public String getLxdh() {
        return lxdh;
    }

    public void setLxdh(String lxdh) {
        this.lxdh = lxdh;
    }

    public String getProvince() {
        return province;
    }

    public void setProvince(String province) {
        this.province = province;
    }

    public String getCity() {
        return city;
    }

    public void setCity(String city) {
        this.city = city;
    }

    public String getDistrict() {
        return district;
    }

    public void setDistrict(String district) {
        this.district = district;
    }

    public String getLxdz() {
        return lxdz;
    }

    public void setLxdz(String lxdz) {
        this.lxdz = lxdz;
    }

    public String getYzbm() {
        return yzbm;
    }

    public void setYzbm(String yzbm) {
        this.yzbm = yzbm;
    }

    public Calendar getApplyTime() {
        return applyTime;
    }

    public void setApplyTime(Calendar applyTime) {
        this.applyTime = applyTime;
    }

    public String getPhone() {
        return phone;
    }

    public void setPhone(String phone) {
        this.phone = phone;
    }

    public String getXm() {
        return xm;
    }

    public void setXm(String xm) {
        this.xm = xm;
    }

    public String getXb() {
        return xb;
    }

    public void setXb(String xb) {
        this.xb = xb;
    }

    public String getCc() {
        return cc;
    }

    public void setCc(String cc) {
        this.cc = cc;
    }

    public String getSfzh() {
        return sfzh;
    }

    public void setSfzh(String sfzh) {
        this.sfzh = sfzh;
    }

    public String getTxzh() {
        return txzh;
    }

    public void setTxzh(String txzh) {
        this.txzh = txzh;
    }

    public String getTxzfzjg() {
        return txzfzjg;
    }

    public void setTxzfzjg(String txzfzjg) {
        this.txzfzjg = txzfzjg;
    }

    public String getLqnf() {
        return lqnf;
    }

    public void setLqnf(String lqnf) {
        this.lqnf = lqnf;
    }

    public String getRxrq() {
        return rxrq;
    }

    public void setRxrq(String rxrq) {
        this.rxrq = rxrq;
    }

    public String getYxmc() {
        return yxmc;
    }

    public void setYxmc(String yxmc) {
        this.yxmc = yxmc;
    }

    public String getZymc() {
        return zymc;
    }

    public void setZymc(String zymc) {
        this.zymc = zymc;
    }

    public String getByrq() {
        return byrq;
    }

    public void setByrq(String byrq) {
        this.byrq = byrq;
    }

    public String getBjyjl() {
        return bjyjl;
    }

    public void setBjyjl(String bjyjl) {
        this.bjyjl = bjyjl;
    }

    public String getXxzjmc() {
        return xxzjmc;
    }

    public void setXxzjmc(String xxzjmc) {
        this.xxzjmc = xxzjmc;
    }

    public String getXxzjhm() {
        return xxzjhm;
    }

    public void setXxzjhm(String xxzjhm) {
        this.xxzjhm = xxzjhm;
    }

    public String getByzsmc() {
        return byzsmc;
    }

    public void setByzsmc(String byzsmc) {
        this.byzsmc = byzsmc;
    }

    public String getZsbh() {
        return zsbh;
    }

    public void setZsbh(String zsbh) {
        this.zsbh = zsbh;
    }

    public String getZmnf() {
        return zmnf;
    }

    public void setZmnf(String zmnf) {
        this.zmnf = zmnf;
    }

    public String getZmwm() {
        return zmwm;
    }

    public void setZmwm(String zmwm) {
        this.zmwm = zmwm;
    }

    public String getZmwh() {
        return zmwh;
    }

    public void setZmwh(String zmwh) {
        this.zmwh = zmwh;
    }

    public String getShjl() {
        return shjl;
    }

    public void setShjl(String shjl) {
        this.shjl = shjl;
    }

    public String getShyj() {
        return shyj;
    }

    public void setShyj(String shyj) {
        this.shyj = shyj;
    }

    public boolean isChange() {
        return change;
    }

    public void setChange(boolean change) {
        this.change = change;
    }

    public Calendar getDysj() {
        return dysj;
    }

    public void setDysj(Calendar dysj) {
        this.dysj = dysj;
    }

    public Calendar getBjsj() {
        return bjsj;
    }

    public void setBjsj(Calendar bjsj) {
        this.bjsj = bjsj;
    }

    public String getReceivePaper() {
        return receivePaper;
    }

    public void setReceivePaper(String receivePaper) {
        this.receivePaper = receivePaper;
    }

    public String getShyy() {
        return shyy;
    }

    public void setShyy(String shyy) {
        this.shyy = shyy;
    }

    public String getYjzqsj() {
        return yjzqsj;
    }

    public void setYjzqsj(String yjzqsj) {
        this.yjzqsj = yjzqsj;
    }

    public String getExCompany() {
        return exCompany;
    }

    public void setExCompany(String exCompany) {
        this.exCompany = exCompany;
    }

    public String getExNumber() {
        return exNumber;
    }

    public void setExNumber(String exNumber) {
        this.exNumber = exNumber;
    }

    public String getCzrbz() {
        return czrbz;
    }

    public void setCzrbz(String czrbz) {
        this.czrbz = czrbz;
    }

    public Calendar getShsj() {
        return shsj;
    }

    public void setShsj(Calendar shsj) {
        this.shsj = shsj;
    }

    public Calendar getBzsj() {
        return bzsj;
    }

    public void setBzsj(Calendar bzsj) {
        this.bzsj = bzsj;
    }
}
