package com.forever.zhb.search.lucene.impl;
...
public class LuceneSearchClientImpl implements LuceneSearchClient{
	...
	public Page<DocumentVo> luceneSearch(String keyword, int start ,int pageSize) throws Exception{
		...
		/*MultiPhraseQuery multiPhraseQuery=new MultiPhraseQuery("content",);
		...
		//总记录数
		//int totalCount = indexSearcher.search(termRangeQuery,null,Integer.MAX_VALUE).totalHits;
		/*int totalCount = indexSearcher.count(termRangeQuery);
		//搜索
		indexSearcher.search(termRangeQuery,collector);
