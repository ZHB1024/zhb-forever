...
public class SearchUtil {
	...
	public void termQuery() throws Exception {
        IndexReader reader = IndexReader.open(FSDirectory.open(new File(LuceneUtil.INDEX_DIR)));
    ...
    private List<DocumentVo> search(Filter filter, String[] fields, String keyword) {
        ...
        try {
            // 创建索引搜索器,且只读
            IndexReader indexReader = IndexReader.open(FSDirectory.open(new File(LuceneUtil.INDEX_DIR)));
